...
function removeFavorites(event){
    ...
    let movie_id = element.parentNode.parentNode.querySelector('small').textContent
    fetch(BASE_URL + "/favorites/remove/" + movie_id).then(onRemoveResponse).then(onJsonRemoveFavorites);
}
...
function onAddMsgJson(json){
    let share =document.querySelector('img.shared-true');
    if(json.op === true){
        share.classList.remove('shared-true');
...
function addMessage(event){
    ...
    document.querySelector('img.shared').classList.add('shared-true');
...
function openModal(event){
    let exists_shared=document.querySelector('img.shared');
    if(exists_shared !==null){
        exists_shared.classList.remove('shared');
    }
...
function onytJson(json){
    ...
    if(json.items.length === 0){
        ...
        not_found.textContent = "Siamo spiacenti non sono stati trovati trailer del film cercato";
        ...
        iframe.src = "https://www.youtube-nocookie.com/embed/" + json.items[0].id.videoId + "?autoplay=1"
...
if(page_count>1){
    ...
    previous_page.textContent = "pagina precedente";
    button_container.appendChild(previous_page);
    previous_page.addEventListener('click',previousPageRequest);
}
...
function onAddListJson(json){
    ...
    if(json.op === true){
        ...
        button.removeEventListener('click',addList);
